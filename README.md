...
初始化使用新浪前复权日线；每日更新使用新浪全市场快照，两者的成交量都从股转换为手。新浪快照不提供换手率，因此增量日线的换手率为空。盘中扫描仍使用东方财富实时行情。项目不自动切换数据源。字段单位依据 [AKShare 股票数据文档](https://akshare.akfamily.xyz/data/stock/stock.html)。
