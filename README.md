...
首次初始化通过新浪下载最近 250 个交易日，使用 3 个下载线程，数据统一由主线程保存。下载失败的股票会记录到日志及 `data/market.json`；部分失败命令返回非零，可直接重跑。已有目标日期数据的股票会自动跳过。有效日线不足 120 条的股票不入池。

## 数据来源

初始化使用新浪前复权日线；每日更新使用新浪全市场快照，两者的成交量都从股转换为手。新浪快照不提供换手率，因此增量日线的换手率为空。盘中扫描也使用新浪，但只按批次请求本地低位股票池，不重复下载全市场行情。项目不自动切换数据源。字段单位依据 [AKShare 股票数据文档](https://akshare.akfamily.xyz/data/stock/stock.html)。
...
更新只请求一次新浪全市场快照，再批量追加目标交易日尚未保存的数据。同一天重跑时已有数据自动跳过。快照缺失或停牌的股票不参与当日股票池。

页面默认手动，点击“扫描一次”；选择“自动”后在交易时段每 60 秒扫描，切回手动停止定时扫描。查看详情时继续自动扫描，关闭浏览器会话停止。非交易时段显示已有结果，不新增盘中信号。显示过滤不改变已保存记录。

新浪批量报价不直接提供量比和换手率。盘中量比按“当前累计成交量 / 过去 20 日均量 / 当日已交易时间比例”估算；换手率根据最近 120 日有效成交量与换手率反推流通盘后估算。因此页面中的这两个盘中指标是筛选近似值，不是交易所原始字段。
