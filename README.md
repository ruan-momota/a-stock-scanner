...
初始化使用新浪前复权日线；成交量从股转换为手，换手率从比例转换为百分数。每日更新和盘中扫描使用东方财富全市场快照。项目不自动切换数据源。字段单位依据 [AKShare 股票数据文档](https://akshare.akfamily.xyz/data/stock/stock.html)。
...
更新只请求一次东方财富全市场快照，再批量追加目标交易日尚未保存的数据。同一天重跑时已有数据自动跳过。快照缺失或停牌的股票不参与当日股票池。
